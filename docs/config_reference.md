...
| `ENABLE_MARKET_STRUCTURE_FEATURES` | Enables the market-structure feature family. | `true` or `false`. | Disable when you want to remove support/resistance, breakout, 52-week, trend-persistence, and structure-count features. |
| `MARKET_STRUCTURE_WINDOWS` | Rolling windows used for normalized distance-to-high and distance-to-low features. | List of positive integers such as `[20, 60, 120, 252]`. | Change when you want shorter or longer support/resistance context. |
| `BREAKOUT_WINDOWS` | Rolling windows used for breakout and breakdown features. | List of positive integers such as `[20, 60, 120]`. | Change when you want different breakout horizons. |
| `MARKET_STRUCTURE_COUNT_WINDOW` | Window used for rolling higher-high / lower-low counts. | Positive integer. | Change when you want shorter or longer structure persistence counts. |
| `NEAR_52W_THRESHOLD` | Threshold for the `near_52w_high` and `near_52w_low` flags. | Float from `0.0` to `1.0`. | Lower it for stricter proximity, raise it for looser proximity. |
| `VOLUME_CONFIRMATION_WINDOW` | Rolling window used for breakout and breakdown volume confirmation. | Positive integer. | Change when volume confirmation should react faster or slower. |
| `ATR_WINDOWS` | ATR windows generated by the market-structure block. | List such as `[14, 20]`. | Change when you want different volatility horizons. |
| `TREND_WINDOWS` | Windows used for `rolling_volatility_*` and `trend_persistence_*` features. | List such as `[20, 60]`. | Change when you want different short/medium trend views. |
| `MARKET_STRUCTURE_LAGS` | Lags used for key market-structure signal lag features. | List such as `[1, 3, 5, 10, 20]`. | Change when sequence models should see different delayed structure signals. |

Current market-structure outputs include:

- normalized distance-to-high and distance-to-low features for each configured window
- breakout and breakdown flags for each configured breakout window
- `higher_high`, `lower_high`, `higher_low`, `lower_low`, and their rolling counts
- `distance_to_52w_high`, `distance_to_52w_low`, `near_52w_high`, and `near_52w_low`
- `breakout_volume_ratio`, `breakdown_volume_ratio`, `volume_spike_ratio`, and `volume_momentum`
- `atr_14`, `atr_20`, `atr_ratio`, `rolling_volatility_20`, `rolling_volatility_60`
- `trend_strength_score`, `trend_persistence_20`, `trend_persistence_60`
- lagged versions of key structure signals such as `higher_high_lag_1` and `trend_strength_score_lag_1`

Behavior:

- only missing market-structure columns are generated
- existing dataset columns are preserved
...
| Field | Meaning | How to set | When to change |
|---|---|---|---|
| `USE_CANDLESTICK_PATTERNS` | Master switch for the custom candlestick feature family. | `true` or `false`. | Disable when you want to remove candle-shape, gap, and binary candlestick pattern inputs from the dataset. |

Current generated candlestick outputs include:

- candle-shape features such as `body_size`, `upper_shadow`, `lower_shadow`, `body_ratio`, `close_position`, and `open_position`
- gap and overnight features such as `gap_up`, `gap_down`, `gap_size`, and `overnight_return`
- ATR-based, breakout, and rolling-range features such as `atr`, `atr_14`, `atr_20`, `rolling_volatility`, `support_distance`, `resistance_distance`, `breakout_signal`, `rolling_high_low_range_5`, and `rolling_high_low_range_20`
- return and volume features such as `return_1d`, `return_5d`, `return_20d`, `volume_momentum`, and `volume_spike`
- lag and rolling sequence features such as `lag_1`, `lag_3`, `lag_5`, `lag_10`, `lag_20`, `rolling_mean_5`, `rolling_std_5`, `rolling_min_5`, and `rolling_max_5`
- binary pattern columns:
  ...
  - `morning_star`
  - `evening_star`
- rolling mean/std/z-score columns for `body_size`, `upper_shadow`, and `lower_shadow` over windows `5`, `10`, `20`, and `60`
- existing dataset columns are preserved; the pipeline only adds the missing features
...
### `model.postgres_logging`

| Field | Meaning | How to set | When to change |
|---|---|---|---|
| `ENABLED` | Whether runtime scripts write run-summary rows into PostgreSQL. Applies to `train`, `test`, `validate`, and `backtest`. | `true` or `false`. | Set to `false` when you want no PostgreSQL inserts for local experiments or offline work. |

Current behavior:

- inserts target `training_runs`, `test_runs`, `validation_runs`, and `backtest_runs`
- logging is best-effort, so database failure does not stop the main run
- stored rows contain config snapshots, dataset metadata, metrics, paths, and failure details
