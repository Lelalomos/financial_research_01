...
def test_train_uses_precomputed_sequences_mode_as_lazy_streaming(tmp_path, monkeypatch):
    ...
    def fail_load_sequences(*_args, **_kwargs):
        raise AssertionError("load_sequences should not be called in lazy precomputed mode")
...
def test_train_uses_on_the_fly_sequences_mode_as_eager_build(tmp_path, monkeypatch):
    module = _load_train_script()
    args = _make_args(tmp_path)
    data_dir = Path(args.data_dir)
    data_dir.mkdir(parents=True, exist_ok=True)
    ...
    def fail_load_sequences(*_args, **_kwargs):
        raise AssertionError("load_sequences should not be called in eager on_the_fly mode")

    def fail_lazy_loader(**_kwargs):
        raise AssertionError("create_lazy_data_loaders should not be called in on_the_fly mode")
